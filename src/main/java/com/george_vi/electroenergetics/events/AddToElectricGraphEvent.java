...
public class AddToElectricGraphEvent extends Event {
    ...
    public AddToElectricGraphEvent(Map<Node, List<Node>> adjacency, Map<DirectionSensitiveNodeConnection, ElectricalProperties> connectionProperties, List<Node> allNodes) {
    ...
    public Node addNode() {
        Node node = new Node(0, new BlockPos(Integer.MAX_VALUE, 0, nodes));
        allNodes.add(node);
        return node;
    }
