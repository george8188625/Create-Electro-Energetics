...
public abstract class FuseHoldable {
    ...
    abstract void preTick(CompoundTag data, int id1, int id2, BridgeCollector.Builder bridges, Level level, BlockPos pos);

    void postTick(CompoundTag data, int id1, int id2, SimulationResults results, Level level, BlockPos pos) {};

    abstract ItemStack getDrops(CompoundTag data);

    @OnlyIn(Dist.CLIENT)
    abstract void render(CompoundTag data, PoseStack pose, MultiBufferSource buffer, int light);
    ...
    void onPlace(CompoundTag data, ItemStack stack, Level level, BlockPos pos) {};

    public static class CopperConductor extends FuseHoldable {

        @Override
        void preTick(CompoundTag data, int id1, int id2, BridgeCollector.Builder bridges, Level level, BlockPos pos) {
            bridges.resistor(id1, id2, 0.1);
        }

        @Override
        ItemStack getDrops(CompoundTag data) {
            return Items.COPPER_INGOT.getDefaultInstance();
        }

        @Override
        void render(CompoundTag data, PoseStack pose, MultiBufferSource buffer, int light) {
    ...
    public static class Fuse extends FuseHoldable {
        ...
        @Override
        ItemStack getDrops(CompoundTag data) {
            return data.getBoolean("Broken") ? CEEBlocks.BROKEN_FUSE.asStack() : CEEBlocks.FUSE.asStack();
        }
